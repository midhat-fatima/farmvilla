...
class Essential_Grid {
	...
	public function enqueue_scripts() {
		...
		if(get_option('tp_eg_use_lightbox') !== 'disabled') {
			if ($esg_dev_mode) {
				wp_register_script( 'themepunchboxext', EG_PLUGIN_URL . 'public/assets/js/dev/esgbox.js', array('jquery'), self::VERSION, $js_to_footer);
			} else {
				wp_register_script( 'themepunchboxext', EG_PLUGIN_URL . 'public/assets/js/esgbox.min.js', array('jquery'), self::VERSION, $js_to_footer);
			}
		}
		...
		wp_register_script( 'tp-tools', EG_PLUGIN_URL . 'public/assets/js/rbtools.min.js', $waitfor, ESG_TP_TOOLS, $js_to_footer );

		if ($esg_dev_mode) {
			wp_register_script( $this->plugin_slug . '-essential-grid-script', EG_PLUGIN_URL . 'public/assets/js/dev/essential-grid.js', array( 'jquery', 'tp-tools' ), self::VERSION, $js_to_footer );
		} else {
			wp_register_script( $this->plugin_slug . '-essential-grid-script', EG_PLUGIN_URL . 'public/assets/js/esg.min.js', array( 'jquery', 'tp-tools' ), self::VERSION, $js_to_footer );
		}
	...
	public static function register_shortcode($args, $mid_content=null){
		...
		if ( $grid->is_gallery ) $caching = 'false';
	...
	**/
				...
				case 'instagram':
					...
					if(is_array($public_photos)){
						foreach ($public_photos as $photo) {
							$photo['custom-image-url-full'] = array($photo['custom-image-url']);
							$photo['custom-image-url'] = array($photo['custom-image-url']);
		...
		if(!empty($posts) && count($posts) > 0){
			foreach($posts as $key => $post){
				...
				if($lightbox_mode == 'content' || $lightbox_mode == 'content-gallery' || $lightbox_mode == 'woocommerce-gallery'){
					...
					$item_skin->set_lightbox_addition(array('items' => $lb_add_images, 'thumbs' => $lb_add_images_thumb, 'base' => $lightbox_include_media));
    ...
    public function output_grid_javascript($load_lightbox = false, $is_demo = false){
		...
		echo 'if (document.readyState === "loading") window.addEventListener(\'DOMContentLoaded\',function() {	esginit_'.$this->grid_api_name .'();}); else esginit_'.$this->grid_api_name .'();'."\n";
