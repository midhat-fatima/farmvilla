...
if ( ! class_exists( 'DIVIENHANCER_Admin_Notices' ) ) {

    class DIVIENHANCER_Admin_Notices {
        ...
        public function action_admin_init() {
            $dismiss_option = filter_input( INPUT_GET, 'divienhancer_dismiss', FILTER_SANITIZE_STRING );
            if ( is_string( $dismiss_option ) ) {
                update_option( "divienhancer_dismissed_$dismiss_option", true );
                wp_die();
            }
        }
        ...
        public function action_admin_enqueue_scripts() {
            ...
            wp_enqueue_script(
                'divienhancer-notify',
                plugins_url( 'scripts/notifies.js', __FILE__ ),
                array( 'jquery' )
            );
        }
            ...
            foreach ( explode( ',', self::TYPES ) as $type ) {
                foreach ( $this->admin_notices->{$type} as $admin_notice ) {
                    ...
                    $dismiss_url = add_query_arg( array(
                        'divienhancer_dismiss' => $admin_notice->dismiss_option
                    ), admin_url() );

                    if ( ! get_option( "divienhancer_dismissed_{$admin_notice->dismiss_option}" ) ) {
                        ?><div
                            class="notice divienhancer-notice notice-<?php echo $type;

                            if ( $admin_notice->dismiss_option ) {
                                echo ' is-dismissible" data-dismiss-url="' . esc_url( $dismiss_url );
                            } ?>">

                            <h2><?php echo "DIVI Enhancer $type"; ?></h2>
...
DIVIENHANCER_Admin_Notices::get_instance();
